...
abstract class BaseAdapter<T> private constructor(recyclerView: RecyclerView) : RecyclerView.Adapter<BaseViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        if (position >= getHeaderCount() && position <= itemCount - getFooterCount() - getLoadMoreCount() - 1) {
            ...
            onBindDataViewHolder(holder, data, position)
    ...
    abstract fun onBindDataViewHolder(holder: BaseViewHolder, data: T, position: Int)
    ...
    private fun getLoadMoreCount() = if (mIsLoadingMore) 1 else 0
    ...
    fun setIsLoadMoreEnable(enable: Boolean) {
